...
# cached to disk so a cold start doesn't have to refit every sector's
# model from scratch each time (see startup_cache.py for why)
from startup_cache import load_or_compute
SARIMA_RESULTS = load_or_compute("sarima_results", fit_sarima_for_all_sectors)
print("SARIMA models ready:", {k: v["order"] for k, v in SARIMA_RESULTS.items()})
